export const generateText = (): string[] => {
  ...
  const firstMultilineTextPart = [multilineTextPart, singleTextLine].join("\n");

  const multilineTextLines = seedArr.map((i) => multilineTextPart);

  multilineTextLines.splice(0, 0, firstMultilineTextPart);
