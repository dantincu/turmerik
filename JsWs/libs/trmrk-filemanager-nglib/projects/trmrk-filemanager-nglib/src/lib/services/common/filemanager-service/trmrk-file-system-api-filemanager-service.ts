...
export interface TrmrkFileSystemApiFileManagerServiceDbObjects {
  tran: IDBTransaction;
...
interface GetItemDetailsOptsCore {
  ...
  intIdnf: number;
  dbStore: (dbObjs: TrmrkFileSystemApiFileManagerServiceDbObjects) => RefLazyValue<IDBObjectStore>;
...
> {
      ...
      for (let j = 0; j < itemsArr.length; j++) {
        ...
        if ((item.Idnf ?? null) === null) {
          ...
          if ((dbItem.Idnf ?? null) === null) {
            intIdnf = await this.execTran(wka, async (dbObjs) => {
              dbItem = (
              ...
              if (dbItem) {
                ...
                return (await dbRequestToPromise(dbObjs.items!.value.put(resolvedItem)))
                  .value as number;
              }
            });
          } else {
  ...
  override async readFolderChildIdnfs(
    ...
    for (let i = 0; i < pathItemsMx.length; i++) {
      ...
      const dbFolder = await this.execTran(
        wka,
        async (dbObjs) =>
          (
            await dbRequestToPromise(dbObjs.folderChildrenIdnfs!.value.get([intIdnf]))
  ...
  private async copyEntriesCore(
    ...
    for (let { itemsArr, areFiles } of [
      ...
      const onEntryCreated = async (i: number, isFile: boolean) => {
        ...
        await this.execTran(wka, async (dbObjs) => {
          const itemIntIdnf = (
            await dbRequestToPromise(
              dbObjs.items!.value.put({
                Name: srcItem.Name,
                clientFetchTmStmpMillis: timeStamp,
          ...
          await dbRequestToPromise(
            dbObjs.folderChildrenIdnfs!.value.put({
        ...
        await this.getDirHandle(
          ...
          async (prDirHandle) => {
            await this.execTran(wka, async (dbObjs) => {
              ...
              await dbRequestToPromise(dbObjs.folderChildrenIdnfs!.value.delete([prItemIntIdnf]));
              await dbRequestToPromise(dbObjs.items!.value.delete([intIdnf]));
            });
        ...
        async (prDirHandle) => {
          ...
          try {
            ...
            await writeFileContentFromBuffObsToIndexedDbIfReq({
              ...
              withTran: (action) =>
                this.execTran(wka, (dbObjs) =>
                  action({
                    fileContentRefs: dbObjs.fileContentRefs!,
                    fileContents: dbObjs.fileContents!,
                  })
                ),
      ...
      await this.getDirHandle(
        ...
        async (prDirHandle) => {
          ...
          await this.execTran(wka, async (dbObjs) => {
            ...
            await dbRequestToPromise(
              dbObjs.folderChildrenIdnfs!.value.delete([
  ...
  private readNamesCore(
    ...
    return this.execTran(wka, async (dbObjs) => {
      const retArr: DriveEntryCore[] = [];
      ...
      for (let i = 0; i < pathItemsMx.length; i++) {
        ...
        const dbItem = (await dbRequestToPromise(dbObjs.items!.value.get([intIdnf])))
          .value as IntIdnfItem;
  ...
  private async readItemDetailsCore(
    ...
    for (let i = 0; i < retEntriesArr.length; i++) {
      ...
      const optsCore: GetItemDetailsOptsCore = {
        ...
        pathItemsArr,
        intIdnf,
        dbStore: (dbObjs) => dbObjs.itemFileSizes!,
        isFile: {
          value: isFile,
        },
        timeStamp,
  ...
  private async getItemDetailsCore<TDbItem extends IntIdnfItemCore>(
    ...
    if (opts.condition) {
      ...
      if (opts.canBeFolder !== false || isFile.value !== false) {
        dbItem = await this.execTran(
          opts.wka,
          async (dbObjs) =>
            (
              await dbRequestToPromise(opts.dbStore(dbObjs).value.get([opts.intIdnf]))
            ).value as TDbItem
        );
  ...
  private async getFolderChildrenArr(
    ...
    await this.getDirHandle(
      ...
      async (dirHandle) => {
        for await (const [key, value] of dirHandle.entries()) {
          await this.execTran(wka, async (dbObjs) => {
            ...
            if (this.dbItemIsStale(dbItem, forceRefresh)) {
              ...
              dbItem.Idnf = (await dbObjs.items!.value.put(dbItem)).result as number;
            }
        ...
        await this.execTran(wka, async (dbObjs) => {
          await dbObjs.folderChildrenIdnfs!.value.put({
            Idnf: intIdnf,
            subFolderIdnfs,
            folderFileIdnfs,
            clientFetchTmStmpMillis: timeStamp,
          ...
          if (!itemHandleRef.value) {
            ...
            await this.execTran(wka, async (dbObjs) => {
              if (dirHandle) {
                ...
                await dbObjs.dirHandles!.value.put(retObj);
                ...
                await dbObjs.fileHandles!.value.put(retObj);
                retVal = await callback(retObj);
              } else {
      ...
      if (!dirHandleRef.value) {
        await this.getDirHandle(
          ...
          async (prDirHandle) => {
            ...
            await this.execTran(wka, async (dbObjs) => {
              await dbObjs.dirHandles!.value.put({
                Idnf: intIdnf,
  ...
  >(
    ...
    if (handleMtblRef.value) {
      ...
      if (error) {
        ...
        await this.execTran(wka, async (dbObjs) => {
          await (isFile.value ? dbObjs.fileHandles : dbObjs.dirHandles)!.value.delete([intIdnf]);
        });
      }
    }
  ...
  private getDbItemHandleCore(
    ...
    return this.execTran(wka, async (dbObjs) => {
      ...
      if (isFile.value === false) {
        dbItem = (await dbRequestToPromise(dbObjs.dirHandles!.value.get([intIdnf]))).value;
      } else if (isFile.value === true) {
        dbItem = (await dbRequestToPromise(dbObjs.fileHandles!.value.get([intIdnf]))).value;
      } else {
        dbItem =
          (await dbRequestToPromise(dbObjs.dirHandles!.value.get([intIdnf]))).value ??
          (await dbRequestToPromise(dbObjs.fileHandles!.value.get([intIdnf]))).value;
  ...
  private async execTran<T>(
    wka: TrmrkFileSystemApiFileManagerServiceWorkArgs,
    action: (dbObjs: TrmrkFileSystemApiFileManagerServiceDbObjects) => Promise<T>
  ) {
    const dbObjs = {} as TrmrkFileSystemApiFileManagerServiceDbObjects;
    ...
    >(
      [
        [
          ...
          (factory) => (dbObjs.folderChildrenIdnfs ??= factory()),
        ],
        [wka.useItems, dbStores.items.dbStoreName, (factory) => (dbObjs.items = factory())],
        [
          wka.useItemFileSizes,
          dbStores.itemFileSizes.dbStoreName,
          (factory) => (dbObjs.itemFileSizes = factory()),
        ],
        [
          wka.useItemTimeStamps,
          dbStores.itemTimeStamps.dbStoreName,
          (factory) => (dbObjs.itemTimeStamps = factory()),
        ],
        [
          wka.useTextFileContents,
          dbStores.textFileContents.dbStoreName,
          (factory) => (dbObjs.textFileContents = factory()),
        ],
        [
          wka.useDirHandles,
          dbStores.dirHandles.dbStoreName,
          (factory) => (dbObjs.dirHandles = factory()),
        ],
        [
          wka.useFileHandles,
          dbStores.fileHandles.dbStoreName,
          (factory) => (dbObjs.fileHandles = factory()),
        ],
        [
          wka.useFileContents,
          dbStores.fileContents.dbStoreName,
          (factory) => (dbObjs.fileContents = factory()),
        ],
        [
          wka.useFileContentRefs,
          dbStores.fileContentRefs.dbStoreName,
          (factory) => (dbObjs.fileContentRefs = factory()),
        ],
      ],
      (argsArr) => {
        for (let arg of argsArr) {
          ...
          if (addStore) {
            ...
            assignStoreFunc(
              () => new RefLazyValue<IDBObjectStore>(() => dbObjs.tran!.objectStore(storeName))
            );
          }
        }
      }
    ...
    dbObjs.tran ??= wka.db!.transaction(dbStoreNames, 'readwrite');
    const retVal = await action(dbObjs);
    return retVal;
  }
}
