...
> {
  ...
  longPressInitArgs: (rootEl: TRootHtmlElement) => LongPressServiceInitArgs
}

export default function TrmrkLongPressable<
  T extends React.ElementType,
  TRootHtmlElement extends HTMLElement = HTMLElement,
>({ hoc, longPressInitArgs }: Readonly<TrmrkLongPressableProps<T, TRootHtmlElement>>) {
  ...
  const rootElAvailable = (rootEl: TRootHtmlElement | null) => {
    ...
    actWithValIf(rootEl, rootEl => longPressService!.init(longPressInitArgs(rootEl)));
