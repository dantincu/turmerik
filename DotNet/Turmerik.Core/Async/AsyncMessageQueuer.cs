...
namespace Turmerik.Core.Async
{
    ...
    public class AsyncMessageQueuer<T> : IAsyncMessageQueuer<T>
    {
        ...
        {
            valueCallback = opts.ValueCallback ?? throw new ArgumentNullException(
                nameof(valueCallback));

            unhandledErrorCallback = opts.UnhandledErrorCallback ?? throw new ArgumentNullException(
                nameof(unhandledErrorCallback));

            alwaysCallback = opts.AlwaysCallback ?? throw new ArgumentNullException(
                nameof(alwaysCallback));

            spinWaitCount = opts.SpinWaitCount;
            waitMillis = opts.WaitMillis;
        }
