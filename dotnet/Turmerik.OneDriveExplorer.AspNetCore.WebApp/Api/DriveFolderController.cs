...
namespace Turmerik.OneDriveExplorer.AspNetCore.WebApp.Api
{
    ...
    {
        public DriveFolderController(IDriveExplorerService driveExplorerService) : base(driveExplorerService)
        {
        }
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> Put(string id, DriveItem driveItem)
        {
            var actionResult = await ExecuteAsync(
                async () =>
                {
