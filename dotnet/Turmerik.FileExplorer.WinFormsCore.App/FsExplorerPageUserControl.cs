...
namespace Turmerik.FsUtils.WinForms.App
{
    public partial class FsExplorerPageUserControl : UserControl
    {
        ...
        public FsExplorerPageUserControl()
        {
            ...
            InitFsDirectoryEntriesGridUserControlEvents();
            InitFsFileEntriesGridUserControlEvents();
        }
        ...
        private void InitFsFileEntriesGridUserControlEvents()
        {
        ...
        private void InitSelectedDirectoryOptsFormCommands(FsEntryOptionsForm form)
        {
        ...
        {
            var form = new FsEntryOptionsForm();
            form.SetCurrentFolderViewModel(viewModel);

            InitCurrentDirectoryOptsFormCommands(form);
            form.Show();
        }

        private void FsDirectoryEntriesGridUserControl_OnFsEntryOptsOpen(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            var form = new FsEntryOptionsForm();
            form.SetParentFolderViewModel(viewModel);

            form.SetEntryName(kvp.Value.Data.Name);
            form.SetIsFolder(true);

            InitSelectedDirectoryOptsFormCommands(form);
            form.Show();
        }

        private void FsFileEntriesGridUserControl_OnFsEntryOptsOpen(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            var form = new FsEntryOptionsForm();
            form.SetParentFolderViewModel(viewModel);

            form.SetEntryName(kvp.Value.Data.Name);
            form.SetIsFolder(false);

            InitSelectedFileOptsFormCommands(form);
            form.Show();
        }

        private void FsFileEntriesGridUserControl_OnReload(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            viewModel.TryExecute(FsExplorerViewModel.ActionNames.ReloadToFolder,
                () => viewModel.ReloadCurrentFolder(), true);
        }

        private void FsDirectoryEntriesGridUserControl_OnReload(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            viewModel.TryExecute(FsExplorerViewModel.ActionNames.ReloadToFolder,
                () => viewModel.ReloadCurrentFolder(), true);
        }

        private void FsFileEntriesGridUserControl_OnGoForward(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            viewModel.TryExecute(FsExplorerViewModel.ActionNames.NavigateToHistoryForward,
                () => viewModel.NavigateToHistoryForward(), true);
        }

        private void FsFileEntriesGridUserControl_OnGoBack(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            viewModel.TryExecute(FsExplorerViewModel.ActionNames.NavigateToHistoryBack,
                () => viewModel.NavigateToHistoryBack(), true);
        }

        private void FsDirectoryEntriesGridUserControl_OnGoForward(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            viewModel.TryExecute(FsExplorerViewModel.ActionNames.NavigateToHistoryForward,
                () => viewModel.NavigateToHistoryForward(), true);
        }

        private void FsDirectoryEntriesGridUserControl_OnGoBack(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            viewModel.TryExecute(FsExplorerViewModel.ActionNames.NavigateToHistoryBack,
                () => viewModel.NavigateToHistoryBack(), true);
        }

        private void FsFileEntriesGridUserControl_OnGoToRoot(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
        ...
        private void FsDirectoryEntriesGridUserControl_OnGoToRoot(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            if (!viewModel.IsRootFolder)
            {
                viewModel.TryExecute(FsExplorerViewModel.ActionNames.NavigateToParentFolder,
        ...
        private void FsFileEntriesGridUserControl_OnGoToParent(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            if (!viewModel.IsRootFolder)
            {
                viewModel.TryExecute(FsExplorerViewModel.ActionNames.NavigateToParentFolder,
        ...
        private void FsDirectoryEntriesGridUserControl_OnGoToParent(KeyValuePair<int, IFsEntriesDataGridRow> kvp)
        {
            if (!viewModel.IsRootFolder)
            {
                viewModel.TryExecute(FsExplorerViewModel.ActionNames.NavigateToParentFolder,
